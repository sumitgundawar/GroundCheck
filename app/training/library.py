...
def _summary(card: dict) -> dict:
    ...
    return {
        ...
        "novelty_check": bool(card.get("novelty")),
...
def _load(model_id: str):
    ...
    if novelty_file.is_file() and card.get("novelty"):
        stats = {k: v.numpy() for k, v in load_file(str(novelty_file)).items()}
        card = {**card, "_novelty_stats": stats}
    with _lock:
...
def predict(model_id: str, image_bytes: bytes) -> dict:
    ...
    with torch.no_grad():
        ...
        if "_novelty_stats" in card:
            features = novelty.feature_extractor(model, card["architecture"])(x).float().numpy()
            distance = float(novelty.distances(features, card["_novelty_stats"])[0])
    ...
    top = ranked[0]
    unfamiliar = distance is not None and distance > card["novelty"]["cutoff"]
    ...
    return {
        ...
        "threshold": threshold,
        "novelty_distance": None if distance is None else round(distance, 2),
        "novelty_cutoff": None if distance is None else round(card["novelty"]["cutoff"], 2),
